...
contract LibEvaluableTest is Test {
    ...
    function testEvaluableV2KnownHash() external pure {
    ...
    function testEvaluableV2HashDifferent(EvaluableV2 memory a, EvaluableV2 memory b) public pure {
        vm.assume(a.interpreter != b.interpreter || a.store != b.store || a.expression != b.expression);
        assertTrue(a.hash() != b.hash());
    }

    function testEvaluableV2HashSame(EvaluableV2 memory a) public pure {
        EvaluableV2 memory b = EvaluableV2(a.interpreter, a.store, a.expression);
        assertEq(a.hash(), b.hash());
    }

    function testEvaluableV2HashSensitivity(EvaluableV2 memory a, EvaluableV2 memory b) public pure {
        vm.assume(a.interpreter != b.interpreter && a.store != b.store && a.expression != b.expression);
    ...
    function testEvaluableV2ReferenceImplementation(EvaluableV2 memory evaluable) public pure {
        assertEq(LibEvaluable.hash(evaluable), LibEvaluableSlow.hashSlow(evaluable));
    }

    /// Test a known hash so that if the hash function changes, we know.
    function testEvaluableV3KnownHash() external pure {
    ...
    function testEvaluableV3HashDifferent(EvaluableV3 memory a, EvaluableV3 memory b) public pure {
        vm.assume(
    ...
    function testEvaluableV3HashSame(EvaluableV3 memory a) public pure {
        EvaluableV3 memory b = EvaluableV3(a.interpreter, a.store, a.bytecode);
        assertEq(a.hash(), b.hash());
    }

    function testEvaluableV3HashSensitivity(EvaluableV3 memory a, EvaluableV3 memory b) public pure {
        vm.assume(
            a.interpreter != b.interpreter && a.store != b.store && keccak256(a.bytecode) != keccak256(b.bytecode)
        );
    ...
    function testEvaluableV3BytecodeLengthSensitivity() public pure {
    ...
    function testEvaluableV3ReferenceImplementation(EvaluableV3 memory evaluable) public pure {
        assertEq(LibEvaluable.hash(evaluable), LibEvaluableSlow.hashSlow(evaluable));
    }
}
